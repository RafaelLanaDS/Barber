...
import { SERVICES } from '../../firebase/config'
  ...
  return (
    <div className="page">
      <div className="status-bar">
        ...
        <span style={{ color:'var(--muted)', fontSize:11 }}>Olá, {user?.displayName || 'Cliente'}</span>
      </div>
      <div className="page-header">
        <div className="eyebrow">Stúdio Valtinho Barber</div>
      ...
      <div className="page-content">
        ...
        {filtered.map(svc=>(
          <div key={svc.id} className={`service-card ${selected?.id===svc.id?'selected':''}`}
            onClick={()=>setSelected(svc)}>
            <div className="service-icon">{svc.icon}</div>
            <div className="service-info">
              <div className="service-name">{svc.name}</div>
              <div className="service-desc">{svc.description}</div>
              <div style={{ fontSize:11, color:'var(--muted)', marginTop:3 }}>⏱ {svc.duration} min</div>
            </div>
            <div className="service-price">R$ {svc.price}</div>
          </div>
        ))}

        <button className="btn btn-red" onClick={handleContinue} disabled={!selected}>
          ✂ Escolher Horário
        </button>

        {selected && (
          <div style={{ textAlign:'center', fontSize:12, color:'var(--red)', marginTop:-4, paddingBottom:8 }}>
