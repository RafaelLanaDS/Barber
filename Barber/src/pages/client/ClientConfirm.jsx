...
import { createAppointment, addProfit, db } from '../../firebase/config'
  ...
  return (
    <div className="page">
      ...
      <div style={{ flexShrink:0, padding:'28px 20px 10px', textAlign:'center' }}>
        <div style={{
          width:64, height:64, background:'#0F2A1A', border:'2px solid var(--success)',
          borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center',
          margin:'0 auto 12px', fontSize:28, color:'var(--success)',
        }}>✓</div>
        <div style={{ fontSize:20, fontWeight:700, color:'var(--white)' }}>Quase pronto!</div>
        <div style={{ fontSize:12, color:'var(--muted)', marginTop:4 }}>Revise os detalhes e confirme</div>
      </div>
      ...
      <div className="page-content" style={{ paddingBottom:20 }}>
          ...
          {clientData && (
            <>
              <div className="confirm-row">
                ...
                <span className="confirm-val">{clientData.name}</span>
              </div>
              <div className="confirm-row">
                <span className="confirm-key">WhatsApp</span>
                <span className="confirm-val">{clientData.phone}</span>
              </div>
            </>
          )}
        ...
        <div className="info-box">
          ...
          <div className="info-box-text">
            ...
            Ao confirmar, o WhatsApp abre com a mensagem pronta para o barbeiro.
            Só clique em <strong style={{ color:'var(--wp-green)' }}>Enviar</strong>.
          </div>
        </div>

        <button className="btn btn-wp" onClick={handleFinalize} disabled={loading}>
          ...
          {loading ? 'Salvando...' : 'Confirmar & Notificar Barbeiro'}
        </button>

        <button className="btn btn-outline" style={{ margin:'0 16px 8px' }}
