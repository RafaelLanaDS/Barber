...
export default function ClientNav() {
  ...
  return (
    <nav className="bottom-nav">
      <button className={`nav-item ${pathname==='/client/services'?'active':''}`} onClick={()=>navigate('/client/services')}>
      ...
      <button className={`nav-item ${pathname==='/client/schedule'?'active':''}`} onClick={()=>navigate('/client/schedule')}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        Agendar
      </button>
      <button className={`nav-item ${pathname==='/client/history'?'active':''}`} onClick={()=>navigate('/client/history')}>
      ...
      <button className="nav-item" onClick={async()=>{ await logout(); navigate('/') }}>
